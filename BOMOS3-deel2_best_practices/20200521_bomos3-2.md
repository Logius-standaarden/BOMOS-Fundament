...
![alt_text](images/organisatiestructuur.png "Organisatiestructuur")
...
Architectuur/Roadmapping/Benchmarking; Ondersteuning en uitvoeri hiervoor past uitstekend bij een research-organisatie in brede zin (Naast kennisinstituten, ook organisaties zoals CBS voor benchmarking). Met name voor benchmarking geldt dat dit beter bij een externe organisatie belegd kan worden.
...
(Probeer de kosten en baten per stakeholder te kwantificeren.)
...
Verwijzen naar de formele standaard, maar dat leidt tot kosten voor implementaties (zie punt 4.)
...
## 5 tien jaar BOMOS: nieuwe ontwikkelingen
...
*   10 jaar sinds publicatie van BOMOS2
*   Ontwikkelingen hebben niet stilgestaan
*   Sommige ontwikkelingen beïnvloeden de best practices voor beheer en doorontwikkeling van open semantische standaarden. 
*   In dit hoofdstuk zetten we de belangrijkste ontwikkelingen op een rij. 
*   Het hoofdstuk sluit af met een vooruitblik


### 2)	Beweging naar ontologieën 


#### Wat zijn ontologieën?



*   Onderscheid twee soorten ‘semantische standaarden’. 
1. ‘Klassieke’ semantische standaarden. Vaak gericht op uitwisseling van berichten (berichtparadigma). Domein meestal gemodelleerd in UML
2. Semantische standaarden nieuwe stijl: met semantische web technologieën. Gericht op publicatie van gelinkte data. Domein wordt meestal als ontologie gemodelleerd in rdfs/owl.
*   We zien beweging van a naar b. Welke bewegingen? 
    *   Steeds grotere rol van Linked Open Data in Rijksoverheid
    *   Bestaande gemeenschappen die SemTech/LOD verkennen of begonnen zijn met toepassing ervan, zoals de SETU in de uitzendbranche met hun FIT with Ontologies project
    *   (Nieuwe samenwerkingen middels SemTech/LOD)
    *   TNO gaat steeds meer samenwerkingen aan omtrent toepassing van SemTech/LOD en zien 
    *   LOD standaarden op de lijst van gangbare of opkomende open standaarden waarvan het Forum Standaardisatie het gebruik aanbeveelt:
        *   OWL (2016) 
        *   RDF / RDFa (2013/2016)
        *   SHACL (2018)
*   Wat betekent dit voor beheer?
    *   Het goede nieuws: BOMOS maakt geen onderscheid tussen de soorten semantische standaarden. De stappen gelden voor beide soorten. 
    *   Maar de invulling, het hoe, is wel anders. (De invulling was überhaupt voor elke situatie uniek, dus was altijd al een uitdaging. Maar uit het FIT project blijkt dat zelfs met dezelfde gemeenschap de switch naar SemTech/LOD veranderingen met zich mee brengt voor de manier waarop je het beheer en doorontwikkeling vorm geeft.) 
*   Ontologies ontstaan meestal niet in green-field scenario maar in ketens waar al uitwisselingsafspraken zijn. ‘Hergebruik!’ luidt het devies. Leidt tot hogere adoptie.
*   Daarbij ook de waarschuwing: De stap naar SemTech/LOD wordt vaak gezien als syntax switch (van UML/XSD naar RDF). Maar dan heb je geen paradigma-shift. Leidt tot verarming in semantiek van de data. Dus wél de kennis hergebruiken, maar de keuzes zijn gebaseerd op oude techniek.  
*   Impact van SemTech op standaardbeheerders maar ook op standaardgebruikers. 
...
1. Business case, rollen, life cycles en maturity levels: BOMOD
2. benodigde technische expertise: SPARQL endpoints of toch maar RESTful API’s (al dan niet i.c.m. JSON-LD)
3. EL: praktische implementeerbaarheid van ontologie
    1. Modelmatig zo’n beetje perfect, maar implementeerbaarheid lastig
    2. BAG voorbeeld: historie zit in dat model. Ongeveer iedereen maakt daarbij de fout dat je met de SPARQL query de historie bevraagt ipv de huidige graph. SPARQL expert en bedenker ontologie kwamen samen.  
        1. SPARQL expert: Wouter Beek (Kadaster, trippy.db start-up (erfgoedsector, kadaster gebruiken die triple store), part time VU verbonden)
        2. Ontologie-bedenker: Pano Maria (Kadaster, ook politie)
        3. EsL biedt aan: sessie met Pano en Wouter organiseren.
    3. Edustandaarden: Jeroen Hamers (ernstig ziek), Jos van de Arend, Marcia van Oploo (bedenker van play-a-LOD spel). Henk Nijstad (kennisnet, teamleader van deze). Henk vindt fijn dat die edu standaarden in BOMOS komen te staan, dus er is bereidheid


### Instrumenten

Nieuwe tools die het dagelijks leven van BOMOS bedrijvers kleuren, en waarvan het goed is om deze tools ter hulp aan te bieden. 



*   Semantic Treehouse (TNO)
*   Sonnet!
*   Pool Party (RCE)
*   Topbraid Composer (Kennisnet)
*   Enterprise Architect (evt)
*   Vocbench (EC)


### Wenkend perspectief (van TNO)

Gezamelijke semantic web ontologie biedt mogelijkheden tot ontology-driven (keten)processen. Met Smart Connecter wordt dan vrijwel alles een knowledge base. Niet alleen je ERP systeem maar ook services o.b.v. machine learning models, GUIs, sensoren & actuators, zelfs mensen (middels smart connectors aan hun applicaties). Deze beschikbaarheid van informatie stelt hoge eisen aan acces contorll, daarom hebbenwe ontology based access control OBAC ontwikkeld. 
...
Leveranciers zijn de kern van een plugfest. Betrek ze daarom vroegtijdig.
